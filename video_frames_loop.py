...
def hackathon_action(i,image,imagepath, pred_coords, class_IDs, bounding_boxs, scores, box_thresh=0.5):
    """ 
    ...
    try:
      ...
      if i % int(50) == 0 and api_ready:
        ...
        x = threading.Thread(target=facepp_plugin, args=(imagepath,detected_action))
        x.start()
...
def keypoint_detection(i,frame, imagepath, detector, pose_net, ctx=mx.cpu(), axes=None):
    
    global pause_time
    

    x, img = gcv.data.transforms.presets.yolo.transform_test(frame, short=512, max_size=1024)
    x = x.as_in_context(ctx)
    class_IDs, scores, bounding_boxs = detector(x)

    plt.cla()
    pose_input, upscale_bbox = detector_to_simple_pose(img, class_IDs, scores, bounding_boxs,
                                                       output_shape=(1024, 768), ctx=ctx)
...
if __name__ == '__main__':
    ...
    for i in range(startFrame,startFrame+numFrames):
        ...
        imagepath = "../data/0"+str(i)+".jpg"
        img = mx.img.imread(imagepath)
        axes = keypoint_detection(i,img, imagepath, detector, net, ctx, axes=axes)